...
while True:
    ...
    if len(indexes) >=5:
        cv2.putText(img,'Warning Crowded!', (850,50), font, 3, (0,0,255), 5)
        c_count += 1
    ...
    if len(indexes)>0:
        for i in indexes.flatten():
            ...
            if(person<lineCy+6 and person>lineCy-6):
                ...
                header = ["Crowd Count","Date"]
                rows = range(1, 3)
                columns = range(1, 3)
                work_sheet.append(header)
                work_sheet.append([c_count,tup2])
                for row in rows:
                    for col in columns:
            ...
            if count >=5 and count <=10:
                ...
                cv2.putText(img,'(Warning! Meets Approximately Capacity)', (500,1000), font, 3, (0,234,255), 5)
            elif count >= 11 :
                
                cv2.putText(img,'(Danger! Maximum capacity has been reached)', (500,1000), font, 3, (0,0,255), 5)
    ...
    # cv2.putText(img,f'People: {str(count)} / 10', (20,140), font, 3, (0,255,0), 3)
